...
from intellifold.data.tools.common import parse_fasta
...
class SingleHitResult:
    ...
    output_template_seq: Optional[str] = None
        ...
        try:
            ...
            for cid, seq_info in valid_chains.items():
                if cid not in mmcif_to_author_chain_id:
                    continue
                auth_cid = mmcif_to_author_chain_id[cid]
                # avoid duplicate residue numbers which can cause issues in downstream processing
                res_nums = []
                seq = []
                for monomer in seq_info:
                    if monomer.num not in res_nums:
                        res_nums.append(monomer.num)
                        seq.append(CCD_NAME_TO_ONE_LETTER.get(monomer.id, "X"))
                seq = "".join(seq)
...
class TemplateHit:
    ...
    def query_to_hit_mapping(self) -> Mapping[int, int]:
        ...
        hit_index = 0
        query_index = 0
        for residue in self.hit_sequence:
            ...
            # Normal aligned residue, in both query and template. Add to mapping.
                ...
                mapping[query_index] = hit_index
                query_index += 1
                hit_index += 1
        mapping = {q: h + self.start_index for q, h in mapping.items()}
        
        return mapping

    def output_templates_sequence(self, structure_sequence) -> str:
        """Returns the final template sequence."""
        result_seq = ['-'] * len(self.query)
        for query_index, template_index in self.query_to_hit_mapping.items():
            result_seq[query_index] = structure_sequence[template_index]
...
class HHRParser:
    ...
    def _parse_hit(lines: Sequence[str]) -> TemplateHit:
        ...
        return TemplateHit(hit_num, hit_name, cols, probs, query, hit_seq, start, idx_q, idx_h)
        ...
        for i, (h_seq, h_desc) in enumerate(parsed, start=1):
            ...
            hits.append(
                TemplateHit(
                    ...
                    query_seq,
                    # h_seq.upper(),
                    h_seq, ## original case may contain lowercase insertions
                    meta.start - 1,
                    meta.end,
                    meta.length,
                    idx_q,
