...
from intellifold.data.template.template_realign import realign_hit_to_structure
from intellifold.data.tools.kalign import Kalign
...
class TemplateFeatures:
    ...
    def empty_template_features(num_res: int, num_dense: int = 24) -> FeatureDict:
        ...
        template_features = {
            ...
            )[None, ...] * 0,   ## use 0 to indicate empty template, instead of 'GAP'
    ...
    def compute_template_unit_vector(
        ...
        return unit_vector, mask_2d, mask
...
class TemplateHitFilter:
    ...
    def _assess_hit(
        ...
        align_ratio = hit.align_ratio
        t_seq = hit.matching_sequence
        len_ratio = hit.length_ratio
...
class TemplateHitProcessor:
    ...
    def _align_query_to_hit_index_mapping(
        ...
        #### query to hit mapping based on hmmsearch sequence, which may contain gaps and lowercase letters for insertions and deletions.
        query_to_hit_mapping = {}
        hit_index = 0
        query_index = 0
        for residue in hmmsearch_sequence:
        ...
        matching_sequence = hmmsearch_sequence.upper().replace('-', '')
        structure_subseq = target_seq[start_index:end_index]
        
        
        ## if the aligned sequence from hmmsearch does not match the structure sequence, 
        ## need to re-align the query sequence to the structure sequence to get the correct mapping.
        if matching_sequence != structure_subseq:
            if length != len(target_seq):
                query_to_hit_mapping = realign_hit_to_structure(
                    hit_sequence=matching_sequence,
                    hit_start_index=start_index,
                    hit_end_index=end_index,
                    full_length=length,
                    structure_sequence=target_seq,
                    query_to_hit_mapping=query_to_hit_mapping,
                )    
                return target_seq, query_to_hit_mapping, actual_chain_id
                
        query_to_hit_mapping = {q: h + start_index for q, h in query_to_hit_mapping.items()}
    ...
    def process(
        ...
        if self._template_cache_dir:
            ...
            if os.path.exists(cache_path):
                ...
                return SingleHitResult(None, None, None, None, None), track
        else:
            ...
            try:
                ...
                return (
                    SingleHitResult(None, None, f"CIF not found: {cif_path}", None, None),
                    track,
                )

        if not res.mmcif_object:
            return (
                SingleHitResult(None, None, f"Parsing failed for {pdb_id}", None, None),
                track,
            )
        ...
        if hit_date > max_date:
            ...
            return (
                SingleHitResult(None, None, err if strict else None, None, None),
                track,
            )
        ...
        try:
            (
                ...
                hmmsearch_sequence=hit.hit_sequence,
                start_index=hit.start_index,
                end_index=hit.end_index,
                length=hit.length,
            )
            ...
            return SingleHitResult(hit, feats, None, warn, None), track
        except Exception as e:
            return (
                SingleHitResult(None, None, f"Error processing hit: {e}", None, None),
                track,
            )
...
class TemplateHitFeaturizer:
    ...
    def get_templates(
        ...
        for hit in valid_hits:
            ...
            seq = re.sub(r'[a-z]', '', hit.hit_sequence)
